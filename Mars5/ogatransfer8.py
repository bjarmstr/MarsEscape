...
def running(trackMenu, rotary_index):
    global Menu_index, Button_pressed, first, OGArate, OGArateprev
    status = "running"
    led_bulb("green")
    Menu_index = 0
    Button_pressed = 0
    ...
    while status == "running":
        tM,rI = run_selector(trackMenu, rotary_index)
        trackMenu= tM
        rotary_index =rI
...
def run_selector(trackMenu,rotary_index):
    ...
    if (Menu_index == 0):  
        newmenu = 0
        rotary_temp =rotary_status(2,circular,rotary_index) #is this legal rotary_index definition
        rotary_index = rotary_temp
        ...
        if (Button_pressed == 1):
            if (rotary_index == 0):
                rotary_temp = rotary_status(2,circular,rotary_index)
                rotary_index = rotary_temp
                display_shutdown(rotary_index)
                newmenu = 1
            if (rotary_index == 1):
                rotary_index = 0
                newmenu = 2
        ...
        #shutdown menu
        rotary_temp = rotary_status(1,circular,rotary_index)
        rotary_index = rotary_temp
        display_shutdown(rotary_index)
        if (Button_pressed == 1):
            if (rotary_index == 0):
        ...
        if first:
            ...
            first = False
        circular = False
        rotary_temp= rotary_status(15,circular, rotary_index)
        rotary_index = rotary_temp
        logging.debug("before display{}".format(OGArate))
        display_rate(rotary_index)
        if (Button_pressed == 1):
...
def display_shutdown(rotary_index):
    with canvas(device) as draw:
        draw.rectangle(device.bounding_box, outline="white", fill="black")
