...
dots = dotstar.DotStar(board.SCK,board.MOSI, 40, brightness=1, auto_write=False)
    ...
    while connectAll < 6:
        for i in range(6):       #there are 6 squares that could have piping
            if pipepin[i] is not None: #if a pipe exists in this square
                print(pipeDb[i],"threshold for this")#X
...
def running(rotary_index, trackMenu):
    global Menu_index, Button_pressed, first, H2Olevel, OGArate, OGArateprev
    status = "running"
    led_bulb("green")
    Menu_index = 0
    Button_pressed = 0
    ...
    while status == "running":
        ...
        run_selector(trackMenu, rotary_index)
