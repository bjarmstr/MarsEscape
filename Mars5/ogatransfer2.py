...
#from checkdb import query_op_parm, insert_op_parm
...
ASTREAM = {"OGA"} #Assembly's Stream in Redis Db
TANK_STREAM = {"H2Otank"}
ERROR_STREAM ={"OGA_error"}
...
pipepin = (21,99,99,26,99,19)#99 designates no pipe in this location
    ...
    while connectAll < 6:
        for i in range(6):       #there are 6 squares that could have piping
            if pipepin[i] != 99: #if a pipe exists in this square
                threshold = int(query_pipe_db(equip_id, pipeDb[i],c)) #added -query_pipe also changed
                led_bulb("blue")
...
def running():
    ...
    while status == "running":
        ...
        H2Olevel = get_redis(TANK_STREAM)
        run_selector()
...
def led_strip_lvl(colorOn):
    ...
    for dot in range(39,(39-lightdots),-1):
        dots[dot] = (0,0,80)
    print(dotRemainder,"dotRemainder")
